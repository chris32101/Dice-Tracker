  ...
  <div class="Preview">
    ...
    <v-card class="mx-auto" max-width="40%" height="100%">
      <v-card-title class="white--text orange darken-4"
        >Participant List</v-card-title
      >

      <v-card-text class="pt-4">
        <b
          >Listed below are all of the pending users for
          {{ leagueName }} league!</b
        >
      </v-card-text>
          ...
          <v-list-item>
            ...
            <div v-if="isOwner == true">
              ...
              <v-list-item-action>
                <v-btn href="google.com" depressed text>
                  Kick User
                  <v-icon color="orange darken-4" right> mdi-close </v-icon>
                </v-btn>
              </v-list-item-action>
            </div>
            <div v-else>
      ...
      <div v-if="isOwner == true">
        <center>
          ...
          ><span>&nbsp; &nbsp;</span
          ><v-btn id="deleteLeague" @click="deleteLeague()"
            >Delete League</v-btn
          >
        </center>
      </div>
      <div v-else>
        <center>
          <v-btn id="confetti" @click="confetti()" text>🎉</v-btn>
        </center>
      </div>
    </v-card>
  </div>
...
export default {
  ...
  methods: {
    ...
    deleteLeague() {
      axios
        .post(
          "http://127.0.0.1:8000/deleteLeague/",
          {
    ...
    finalizeTeams() {
      ...
      axios
        .post(
          "https://calm-retreat-42630.herokuapp.com/https://nishi7409:1WVHeSGXHOaYXWyNfysXl1NduV4tsNDmgcrfY6hU@api.challonge.com/v1/tournaments.json",
          {
        ...
        .then(function (response) {
          if (response.status == 200) {
            ...
            localStorage.setItem(
              "challongeURLForEmbed",
              response.data.tournament.url
            );
          } else {
      ...
      for (var y = 0; y < JSON.parse(localStorage.getItem("teamLength")); y++) {
        ...
        axios
          .post(
            "http://127.0.0.1:8000/addTeamToLeague/",
            {
      ...
      setTimeout(function () {
        axios
          .post(
            `http://127.0.0.1:8000/saveChallongeData`,
            {
      ...
      setTimeout(function () {
        axios
          .post(
            `https://calm-retreat-42630.herokuapp.com/https://nishi7409:1WVHeSGXHOaYXWyNfysXl1NduV4tsNDmgcrfY6hU@api.challonge.com/v1/tournaments/${localStorage.getItem(
              "challongeLeagueID"
      ...
      setTimeout(function () {
        axios
          .post(
            `http://127.0.0.1:8000/saveMatchIDs`,
            {
              leagueName: localStorage.getItem("leagueName"),
              matchIDs: matchIDs,
            },
    ...
    addDisabled(item, id, index) {
      ...
      if (this.map.get(id) == null) {
        ...
        this.disabledItems.splice(
          this.disabledItems.indexOf(this.map.get(id)),
          1
        );
        this.map.set(id, item);
        this.disabledItems.push(item);
    ...
    axios
      ...
      .then(function (response) {
        if (response.data.response == false) {
          ...
          localStorage.setItem(
            "allUsernamesForLeague",
            JSON.stringify(response.data.error)
          );
          localStorage.setItem(
            "teamLength",
            JSON.stringify(response.data.teamLength)
          );
        }
      });
